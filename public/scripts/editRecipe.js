...
$(document).ready(function() {
    ...
    // Ladataan reseptin tiedot muokkausta varten
    $.ajax({
        url: `/recipes/view/${recipeId}`,
        method: "GET",
        success: function(response) {
            if (response.success && response.recipe) {
                prefillForm(response.recipe);
            } else {
                alert("Reseptin lataaminen epäonnistui: " + response.message);
                window.location.href = "/pages/user-page.html";
            }
        },
        error: function(jqXHR) {
            alert("Reseptin lataaminen epäonnistui: " + (jqXHR.responseJSON?.message || "Tuntematon virhe"));
            window.location.href = "/pages/user-page.html";
        }
    });

    // Esitäytetään lomake reseptin tiedoilla
    function prefillForm(recipe) {
        $("#recipe-name").val(recipe.name);
        $("#recipe-serving-size").val(recipe.servingSize);
        $("#recipe-preparation-time").val(recipe.preparationTime)

        // Esitäytetään ainesosat
        $(".ingredients").empty();
            ...
            const ingredientsHtml = `
                <div class="ingredient">
                    <div class="row align-items-center">
                        <div class="col-3">
                            <input type="text" class="form-control ingredient-amount-and-unit" value="${ing.amountAndUnit}"
                            placeholder="Määrä (esim. 100g)" required>
                        </div>
                        <div class="col-6">
                            <input type="text" class="form-control ingredient-name" value="${ing.ingredientName}"
                            placeholder="Ainesosa" required>
                        </div>
        ...
        selectedTags.forEach(tag => {
            $(`input[value="${tag.value}"]`).prop("check", true);
        });
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    alert("Reseptin päivittäminen epäonnistui: " + response.message);
                }
            },
            error: function(jqXHR) {
                if (jqXHR.status === 0) {
